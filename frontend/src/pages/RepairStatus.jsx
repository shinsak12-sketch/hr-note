...
export default function RepairStatus() {
  ...
  async function handleSearch(e) {
    ...
    setLoading(true);
    try {
      const data = await api.getMyRepairs(form.emp_no, form.password);
      setRequests(data); setSearched(true);
  ...
  return (
    <div className="app-container">
      <div className="header">
        ...
        <div className="header-title">수선 요청 현황</div>
        <div style={{ width: 40 }} />
      </div>
      ...
      <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
        <form onSubmit={handleSearch} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="form-group">
            ...
            <input type="text" placeholder="신청 시 입력한 사번" value={form.emp_no} onChange={e => setForm(f => ({ ...f, emp_no: e.target.value }))} autoCapitalize="none" />
          </div>
          <div className="form-group">
            <label className="form-label">비밀번호 <span className="req">*</span></label>
            <input type="password" placeholder="신청 시 설정한 비밀번호" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} />
          </div>
          {error && <div style={{ color: 'var(--red)', fontSize: 13 }}>{error}</div>}
          <button type="submit" className="btn-primary" disabled={loading} style={{ background: '#5C3D8F' }}>
            {loading ? '조회 중...' : '🔍 조회'}
          </button>
          ...
          return (
            <div key={r.id} style={{ background: 'var(--bg)', border: '0.5px solid var(--border)', borderLeft: `4px solid ${st.color}`, borderRadius: 12, padding: '14px 16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <span style={{ fontWeight: 700, fontSize: 15 }}>{r.repair_type} 수선</span>
